...
export MODEL="${PAPERTOOLS_DAILY_MODEL:-glm}"
export FILTER_MODEL="${PAPERTOOLS_DAILY_FILTER_MODEL:-glm}"
export PAPERTOOLS_FILTER_MODEL_CHAIN="${PAPERTOOLS_DAILY_FILTER_MODEL_CHAIN:-glm}"
export CLUSTER_MODEL="${PAPERTOOLS_DAILY_CLUSTER_MODEL:-glm}"
export PAPERTOOLS_CLUSTER_MODEL_CHAIN="${PAPERTOOLS_DAILY_CLUSTER_MODEL_CHAIN:-glm}"
# Canonical summary chain, shared with daily_full_run.sh: SJTU only by default.
# deepseek-reasoner cannot fit the SJTU token bucket; Prism is opt-in while funded.
export SUMMARY_MODEL_CHAIN="${PAPERTOOLS_DAILY_SUMMARY_MODEL_CHAIN:-sjtu:glm}"
...
export SUMMARY_MODEL="${PAPERTOOLS_DAILY_SUMMARY_MODEL:-glm}"
...
# Wall-clock budget for the summary stage: save progress + exit code 3 (partial)
# instead of spinning until the hard timeout SIGKILL. Shared with daily_full_run.sh.
...
export PAPERTOOLS_DAILY_MAX_ATTEMPTS="${PAPERTOOLS_DAILY_MAX_ATTEMPTS:-3}"
export PAPERTOOLS_DAILY_RETRY_BASE_SECONDS="${PAPERTOOLS_DAILY_RETRY_BASE_SECONDS:-30}"
...
if ! [[ "$PAPERTOOLS_DAILY_MAX_ATTEMPTS" =~ ^[0-9]+$ ]] || [ "$PAPERTOOLS_DAILY_MAX_ATTEMPTS" -lt 1 ]; then
  log "Invalid PAPERTOOLS_DAILY_MAX_ATTEMPTS=$PAPERTOOLS_DAILY_MAX_ATTEMPTS; falling back to 3"
  PAPERTOOLS_DAILY_MAX_ATTEMPTS=3
fi
if ! [[ "$PAPERTOOLS_DAILY_RETRY_BASE_SECONDS" =~ ^[0-9]+$ ]]; then
  log "Invalid PAPERTOOLS_DAILY_RETRY_BASE_SECONDS=$PAPERTOOLS_DAILY_RETRY_BASE_SECONDS; falling back to 30"
  PAPERTOOLS_DAILY_RETRY_BASE_SECONDS=30
fi

max_attempts="$PAPERTOOLS_DAILY_MAX_ATTEMPTS"
attempt=1
while [ "$attempt" -le "$max_attempts" ]; do
  ...
  sleep_seconds=$(( attempt * attempt * PAPERTOOLS_DAILY_RETRY_BASE_SECONDS ))
