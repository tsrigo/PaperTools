...
from src.utils import document_content as document_content_validation

MIN_VALID_PAPER_CONTENT_CHARS = (
    document_content_validation.MIN_VALID_PAPER_CONTENT_CHARS
)
MIN_VALID_PAPER_CONTENT_ALPHA_CHARS = (
    document_content_validation.MIN_VALID_PAPER_CONTENT_ALPHA_CHARS
)
...
class ExtractionResult:
    ...
    def to_cache_payload(
        self, normalized_source: str, ocr_mode: str, cache_version: str
    ) -> Dict[str, Any]:
        return {
...
def normalize_whitespace(text: Any) -> str:
    ...
    return document_content_validation.normalize_whitespace(text)
...
def get_paper_content_issue(content: Optional[str]) -> Optional[str]:
    ...
    return document_content_validation.get_document_content_issue(
        content, enforce_paper_length=True
    )
...
def normalize_document_source(
    source: str, source_type: Optional[str] = None
) -> ExtractionContext:
...
class ExtractionManager:
    ...
    def __init__(
        ...
        self.ocr_mode = (
            (ocr_mode or DOCUMENT_EXTRACT_OCR_MODE or "auto").strip().lower()
        )
        ...
        self.remote_fallback = (
            ...
            else bool(remote_fallback)
        )
        self.request_timeout = request_timeout or DOCUMENT_EXTRACT_TIMEOUT

    def get_cached_result(
        self, source: str, source_type: Optional[str] = None
        ...
        for provider_name in self.chain:
            ...
            try:
                ...
                ensure_valid_extraction_content(
                    result.content, f"cached {provider.name} {source}"
                )
                return result
            except Exception:
                continue
    ...
    def extract(
        ...
        with tempfile.TemporaryDirectory(prefix="papertools-doc-") as temp_dir:
            for provider_name in self.chain:
                ...
                if cached_entry:
                    try:
                        result = ExtractionResult.from_cache_payload(
                            cached_entry["data"]
                        )
                        ensure_valid_extraction_content(
                            result.content, f"cached {provider.name} {source}"
                ...
                try:
                    context_for_provider = self._prepare_context_for_provider(
                        provider, context, temp_dir
                    )
                    result = provider.extract(
                        context_for_provider, ocr_mode=self.ocr_mode
                    )
        ...
        raise DocumentExtractionError(
            "所有文档提取 provider 均失败: "
            + " | ".join(attempt_errors or ["unknown error"])
        )
    ...
    def _prepare_context_for_provider(
        ...
        suffix = get_file_suffix_for_source(
            context.source_type, context.normalized_source
        )
        ...
        response = requests.get(
            context.normalized_source, headers=headers, timeout=self.request_timeout
        )
...
def get_document_content(
    ...
    *,
    ...
    manager = ExtractionManager(
        cache_manager=cache_manager, chain=chain, ocr_mode=ocr_mode
    )
