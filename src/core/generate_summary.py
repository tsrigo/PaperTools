...
---
请按以下两个部分回答，用中文，专业术语保留英文。

### § 1 研究问题与重要性
论文提出并解决的研究问题是什么？为什么这个问题是重要的？解决这个问题能带来哪些价值？如果重要性不显而易见，补充必要的背景。

### § 2 前人工作与不足
这个问题之前被解决了吗？之前的研究为什么存在不足？点名最相关的 prior methods 或 systems，说明它们已经能做到什么，再精确解释它们为什么不够：是方法本身的限制、关键假设失效、工程成本太高、评测缺口，还是当时问题没有被清楚定义？避免泛泛说"prior work 没有考虑 X"——要解释为什么 X 没有被考虑。
...
### § 3 重建作者的思考路径
在正式讲方法之前，先逆向重建作者可能的思考路径。不要用论文自己的贡献作为前提——只能使用这篇论文之前已经存在的背景、失败模式、经验观察、理论线索和相关工作。目标是模拟一个认真研究这个问题的人，如何从已有知识走到这篇论文的 idea。

### § 4 核心 Intuition
用 2–4 句话说清楚这篇论文提出方法的 Intuition 是什么。去掉所有形式化、消融实验和工程细节。让先前方法失效、让这个方法奏效的那个核心是什么？如果用普通语言说不清楚，说明理解还没到位。
...
输出以流畅段落为主。每句话都要有信息量。"""
    ...
    return _llm_generate(
        ...
        "你是一个精确的学术阅读助手。用中文回复，专业术语保持英文。",
        prompt,
        f"core_insight_v3_{paper_title}",
        paper_content,
        cache_manager,
    )
...
### § 7 实验设计与结论
这篇论文是如何设计实验来验证提出的方法和 claim 的？按以下格式总结：提出了什么问题 → 设计了什么实验验证这个问题 → 问题的答案是什么。不需要很多数据细节，只需要核心思路。
...
---
请按以下三个部分回答，用中文，专业术语保留英文。

### § 8 Take-aways
总结这篇论文的 take-aways。每句话都要有信息量，避免大空话。

### § 9 最脆弱的假设
每个方法都建立在假设之上。找出最关键的那一个：如果它不成立，核心贡献会直接失效。精确解释这个假设在实际中可能不成立的原因，以及论文为这个假设提供了什么证据（或缺少什么证据）。不要列举所有局限性——找那个失败代价最大的假设。

### § 10 最小复现实验
如果我有 1 周时间，能做一个最小复现实验验证它的哪一点？说清楚：用什么数据，实现什么，测量什么，什么结果算是支持或反驳了这个 claim。优先选择不需要复现完整系统的方案。
...
按以下结构输出：
§ 8 Take-aways（至少 1 条，必须有信息量）
§ 9 最脆弱的假设（最关键的一个假设，如果不成立则核心贡献直接失效）
§ 10 最小复现实验（用什么数据、实现什么、测量什么、什么结果算支持或反驳）

如果当前文本缺乏实验细节，不要编造数字，基于摘要和引言推断并明确说明。不能输出"无""没有""生成失败"。专业术语保留英文。"""
    return _llm_generate(
        providers,
        temperature,
        "你是一个负责修复论文网页必填字段的学术阅读助手。输出必须真实、有依据、非空。",
        prompt,
        f"additional_insights_repair_v3_{paper_title}",
        repair_context,
        cache_manager,
    )
...
### § 11 最强反例设计
如果我反对这篇论文，我会怎么设计反例？设计对这篇论文最有力的攻击。找一个实验、一个理论论证，或一类真实世界的场景，能真正挑战这个方法是否按声称的那样有效。一个好的反例要么提出对结果的具体替代解释，要么识别出方法会可预测地失败的条件。

### § 12 Follow-up Research Idea
基于对方法核心局限的理解，提出一个非增量的 novel research idea。不是"把方法扩展到领域 X"或"加一个模块处理情况 Y"，而是一个新的 framing、新的目标，或与相邻问题的新连接，能打开一个真正不同的研究方向。说清楚：（a）什么局限或未满足的需求驱动了这个想法，（b）可以借鉴哪个相邻领域的方法或工具，（c）第一个实验大概长什么样。明确区分你的提议和已有工作。
...
输出以流畅段落为主。每句话都要有信息量。"""
    ...
    return _llm_generate(
        ...
        "你是一个精确的学术阅读助手。用中文回复，专业术语保持英文。",
        prompt,
        f"research_value_v2_{paper_title}",
        assembled_input,
        cache_manager,
    )
                ...
                if cached_result and cached_result.content:
                    ...
                    if cached_paper_content:
                        cached_intro_logic = cache_manager.get_summary_cache(
                            f"intro_logic_v3_{paper_title}", cached_paper_content
                        )
