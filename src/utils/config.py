...
# 论文筛选Prompt模板
PAPER_FILTER_PROMPT = """你是一位顶尖的人工智能研究员，正在为一项关于 "LLM智能体及其演化"（LLM-based Agents and their Evolution） 的研究课题筛选前沿论文。请你严格但不要过窄地判断这篇论文是否符合我的研究范围。

我的核心目标: 筛选出那些与 构建、改进、评估、训练或演化 LLM智能体 强相关的论文。我的研究焦点是 Agentic AI，特别是以下三个方向及其子方向：
...
- 保留 (Keep): 如果论文研究 agent 的 harness / scaffold / runtime / tool middleware / system prompt / execution environment / continual harness，并且核心贡献是让这些智能体运行组件通过轨迹、反馈、观测或评估结果自动改进、演化或自适应优化。这里的 harness/scaffold 是智能体能力实现的一部分，不应简单归为普通基础设施。此规则不限于 coding-agent，同样适用于 embodied agent、web agent、game-playing agent 等任何需要长期决策的智能体场景。
- 保留 (Keep): 如果论文的研究对象明确是 LLM Agents / Long-Horizon Agents / Tool-Use Agents / Multi-Agent LLMs，即使它主要是行为分析、评估基准、失败机制诊断或经验研究，只要结论直接帮助理解或改进智能体能力，也应保留。
- 保留 (Keep): 如果论文研究 test-time scaling、推理策略搜索、澄清时机、记忆机制、协作意图、agent training/distillation/RL，并且标题或摘要明确指向 Agentic / LLM Agents / Self-Evolving / LLMs Improving LLMs，应保留。不要因为它看起来像“推理优化”或“训练方法”就直接排除。
- 保留 (Keep): 如果论文的核心机制是 self-evolving / self-improving / iterative improvement / LLMs improving LLMs，即使对象是 instruction following、test-time reasoning 或模型训练，也应优先保留，因为这属于“演化机制”的相关前沿。
- 排除 (Exclude):
  1. 非演化型应用 (Non-Evolving Applications): 如果论文只是将LLM（或一个已有的Agentic / Multi-Agent框架）作为工具应用到特定领域去解决该领域的问题（例如生物、医疗、金融、法律等），且不涉及任何新的智能体机制设计（如 harness 改进、自我改进、记忆演化、技能获取等），应排除。但如果论文在特定领域中提出了新的 agent 机制（如 embodied self-improving harness、game-playing agent 的 continual adaptation），则不应排除——领域是场景，机制才是贡献。
  2. 非Agentic的推理: 如果论文只是关于提高LLM的基础推理能力（如新的CoT变体、逻辑、数学），且标题和摘要都没有体现 Agent / Multi-Agent / Tool Use / Memory / Self-Evolving / LLMs Improving LLMs 等智能体相关机制。
...
- 智能体能力: `Planning`, `Tool Use / Tool Augmentation`, `Memory`, `Self-Correction`, `Self-Reflection`, `ReAct`,  `Collaboration`, `Agent Harness`, `Agent Scaffold`, `Agent Runtime`, `Continual Harness`, `Execution Environment`, `Long-Horizon Decision-Making`, `Embodied Agent`, `Skill Acquisition`
- 演化机制: `Self-Improvement`, `Self-Refine`, `Generational Evolution`, `Iterative Improvement`, `Online Adaptation`, `Prompt Optimization`, `Harness Refinement`, `Co-Learning`
...
- 但如果安全/评估只是论文研究 LLM Agent 能力、行为边界或失败模式的场景，而核心对象仍是智能体能力本身，不要机械排除；先按第一步的 Agentic 相关性判断。
...
重要校准：
- 以下类型应倾向保留：LLMs Improving LLMs / Agentic Discovery / Test-Time Scaling；LLM Agents 的 memory、cooperation、clarification timing、long-horizon behavior；Self-Evolving RL 或 self-improving instruction following；Small Language Model Agents 的 tool-use training/distillation。
- 不要只因为论文是 benchmark、evaluation、behavior analysis、training/distillation 或 test-time optimization 就排除。只要标题和摘要明确围绕 LLM Agents / Agentic AI / Self-Evolving LLMs 展开，就应返回 True。
